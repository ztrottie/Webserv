...
Server::Server(int &listSocket) : _port(8080), _host("10.12.6.9"), _name("default") {
	...
	listSocket = _listSocket;
	std::cout << _name <<  " is listening on port " << _port << std::endl;
}

Server::Server(uint16_t port, const char *host, std::string name, int &listSocket) : _port(port), _host(host), _name(name) {
	std::cout << YELLOW "Initializing a default Server named " << _name << " on " << _host << ":" << _port << RESET << std::endl;
	_listSocket = socket(AF_INET, SOCK_STREAM, 0);
	if (_listSocket == -1)
		throw std::invalid_argument("Socket creation Failed");
...
int Server::acceptConnection() {
	return accept(_listSocket, NULL, NULL);
}